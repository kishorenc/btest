...
#pragma omp parallel for
    ...
    /** Search procedure
      ...
      *
    */
    void IndexIVF_HNSW::search(size_t k, const float *x, float *distances, int64_t *labels)
    {
        ...
        faiss::maxheap_heapify<float>(k, distances, labels);
    ...
    {
        for (size_t i = 0; i < nc; i++) {
            const float *centroid = reinterpret_cast<const float *>(quantizer->getDataByInternalId(i));
    ...
    void IndexIVF_HNSW::rotate_quantizer() {
        ...
        for (size_t i = 0; i < nc; i++){
            float *centroid = reinterpret_cast<float *>(quantizer->getDataByInternalId(i));
    ...
    {
        for (size_t i = 0; i < n; i++) {
            const float *centroid = reinterpret_cast<const float *>(quantizer->getDataByInternalId(keys[i]));
